...
function getClassesFromCalender(
  calender: CalendarResponse
): CalendarComponent[] {
  ...
  cours.sort(
    (a, b) =>
      moment(a.start as DateWithTimeZone).unix() -
      moment(b.start as DateWithTimeZone).unix()
  );
  return cours;
}
...
function getFormattedTeacher(name: string) {
  ...
  let emoji = customEmojis.get(capitalizeName);
